...
// Données statiques pour remplacer les imports contextuels
...
// Données statiques pour le projet
const staticProject = {
  id: 'project-1',
  name: 'Mon Projet Statique',
  startDate: '2024-01-01',
  endDate: '',
  currency: 'EUR',
  currency_symbol: '€',
  display_unit: 'standard',
  decimal_places: 2,
  timezone_offset: 0,
};
...
const ProjectSettingsView = () => {
  ...
  const [name, setName] = useState(staticProject.name);
  const [startDate, setStartDate] = useState(staticProject.startDate);
  const [endDate, setEndDate] = useState(staticProject.endDate);
  const [isEndDateIndefinite, setIsEndDateIndefinite] = useState(!staticProject.endDate);
  
  const [currencyCode, setCurrencyCode] = useState(staticProject.currency);
  const [currencySymbol, setCurrencySymbol] = useState(staticProject.currency_symbol);
  const [isCustomCurrency, setIsCustomCurrency] = useState(false);
  const [displayUnit, setDisplayUnit] = useState(staticProject.display_unit);
  const [decimalPlaces, setDecimalPlaces] = useState(staticProject.decimal_places);
  const [timezoneOffset, setTimezoneOffset] = useState(staticProject.timezone_offset);
  ...
  const handleSaveChanges = () => {
    if (!name.trim() || !startDate) {
      alert('Le nom et la date de début sont requis.');
      return;
    }
    ...
    if (!finalCurrencyCode) {
      alert('Le code de la devise ne peut pas être vide.');
      return;
    }
  ...
  // Pour tester la vue consolidée, décommentez la ligne suivante :
            ...
            {isCustomCurrency && (
              <div className="grid grid-cols-2 gap-4 mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                <div>
                  ...
                  <input 
                    type="text" 
                    value={currencyCode === 'custom' ? '' : currencyCode} 
