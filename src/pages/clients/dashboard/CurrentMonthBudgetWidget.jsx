    ...
    <div className="bg-white p-6 rounded-lg shadow-md">
      ...
      <div className="space-y-6">
        ...
        <div>
          <div className="flex justify-between items-baseline mb-2">
            <h3 className="font-semibold text-gray-700 flex items-center gap-2">
              <TrendingUp className="w-4 h-4 text-green-500" /> Entrées
            </h3>
            <div className="text-sm">
              <span className="font-bold text-green-600">
                {formatCurrency(totalActualIncome, settings)}
              </span>
              <span className="text-gray-500">
                {" "}
                / {formatCurrency(totalBudgetedIncome, settings)}
              </span>
            </div>
          </div>
          <ProgressBar
        ...
        <div>
          <div className="flex justify-between items-baseline mb-2">
            <h3 className="font-semibold text-gray-700 flex items-center gap-2">
              <TrendingDown className="w-4 h-4 text-red-500" /> Sorties
            </h3>
            <div className="text-sm">
              <span className="font-bold text-red-600">
                {formatCurrency(totalActualExpense, settings)}
              </span>
              <span className="text-gray-500">
                {" "}
                / {formatCurrency(totalBudgetedExpense, settings)}
              </span>
            </div>
          </div>
          <ProgressBar
            value={totalActualExpense}
            total={totalBudgetedExpense}
            colorClass="bg-red-500"
          />
          <p className="text-xs text-right mt-1 text-gray-500">
            Reste à dépenser:{" "}
            <span className="font-medium text-gray-700">
              {formatCurrency(remainingExpenseBudget, settings)}
            </span>
          </p>
        </div>
          ...
          <div className="flex justify-between items-center">
            ...
            <span
              className={`text-lg font-bold ${
                netActual >= 0 ? "text-gray-800" : "text-red-700"
              }`}
            >
