...
export const generateTaxPaymentEntries = (actuals, period, taxConfig) => {
    ...
    switch (taxConfig.base_type) {
        ...
        case 'salary':
             ...
             baseAmount = actuals
                .filter(a => a.type === 'payable' && a.category === salarySubCategory)
                .flatMap(a => a.payments || [])
                .filter(p => {
                    const pDate = new Date(p.paymentDate);
                    return pDate >= startDate && pDate < endDate;
    ...
    return [{
        id: `tax_payment_${taxConfig.id}_${period.startDate.toISOString()}`,
        type: 'depense',
        ...
        startDate: paymentDueDate.toISOString().split('T')[0],
        is_tax_payment: true,
        description: `${taxConfig.name} pour la période du ${startDate.toLocaleDateString('fr-FR')} au ${endDate.toLocaleDateString('fr-FR')}`
    }];
};
