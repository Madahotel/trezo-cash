...
const Sidebar = ({ isCollapsed, onToggleCollapse }) => {
  ...
  const { overdueTransactions } = useScheduleData(
    actualTransactions,
    dataState.settings
  );
  ...
  const tooltipContent = useMemo(
    () => ({
      dashboard: {
        title: 'Dashboard',
        description:
          "Vue d'ensemble de la santé financière de votre projet avec indicateurs clés.",
        imageUrl: 'https://i.imgur.com/nJbC2a2.png',
      },
      budget: {
        title: 'Budget',
        description:
          'Créez et gérez votre budget prévisionnel avec suivi des écarts.',
        imageUrl: 'https://i.imgur.com/rF9gYtK.png',
      },
      trezo: {
        title: 'Trezo',
        description:
          'Tableau de bord trésorerie : du quotidien à la vision long terme (10 ans).',
        imageUrl: 'https://i.imgur.com/dAmf2u2.png',
      },
      flux: {
        title: 'Flux',
        description:
          "Analysez l'évolution de votre trésorerie avec des visualisations claires.",
      ...
      analyse: {
        title: 'Analyse',
        description:
          'Analysez vos dépenses et revenus pour des décisions éclairées.',
        imageUrl: 'https://i.imgur.com/jV7fL4c.png',
      },
      scenarios: {
        title: 'Scénarios',
        description:
          "Simulez l'impact de vos décisions sur votre trésorerie future.",
        imageUrl: 'https://i.imgur.com/tY8wP9d.png',
      },
      comptes: {
        title: 'Comptes',
        description:
          'Gérez vos comptes bancaires, caisse et autres instruments de trésorerie.',
  ...
  const settingsItems = useMemo(
    () => [
      ...
      {
        id: '/client/tableau-tva',
        label: 'Tableau TVA',
        icon: Hash,
        color: 'text-green-500',
      },
      {
        id: '/client/emprunts-prets',
  ...
  const navLinkClasses = ({ isActive }) => {
    const baseClasses = `flex items-center w-full h-12 rounded-xl transition-all duration-200 group relative overflow-hidden ${
    ...
    const activeStateClasses = isActive
      ? 'bg-transparent'
      : inactiveStateClasses; // Utilisez `bg-transparent` si actif pour éviter toute classe `bg-*`
    ...
    >
      ...
      <div className="flex items-center justify-between h-19 px-4 py-4 border-b border-gray-200 bg-white/50 backdrop-blur-sm">
        ...
        >
          ...
          >
            <DollarSign className="w-8 h-8" style={{ color: themeActive }} />
          </NavLink>
          <AnimatePresence>
            {!isCollapsed && (
              <motion.span
            ...
            >
              ...
              >
                ...
                <motion.div
                  ...
                  initial={false}
                  animate={{
                    scale: navLinkClasses({ isActive: true }).includes(
                      'bg-blue-500'
                    )
                      ? 1
                      : 0,
